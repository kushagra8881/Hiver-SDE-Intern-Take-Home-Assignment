...
- [ ] Run `make evaluate-live` with a fresh environment-only key, or clearly label fallback outputs.
- [ ] Run `make audit`, complete the blinded human sheet, then `make judge` and `make agreement`.
- [ ] Copy generated metrics into the frozen Results section of `REPORT.md` without cherry-picking.
